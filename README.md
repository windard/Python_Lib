...
*重要更新，在此之前本人的测试机器为windows10,但是在此之后，本人的测试机器换为了ubuntu 15，python仍然是2.7 如果与你的运行结果有差异的话，注意看一下是是不是运行平台的问题*
...
##ftplib
这是python的进行ftp文件传输的库，可以进应用TLS加密，一般的就是创建一个ftp对象ftplib.FTP(),应用加密的话就是创建一个ftp-tls对象ftplib.FTP\_TLS()。  
...
f.quit()
```
保存为ftplib\_demo.py,运行，看一下结果。                
![ftplib\_demo.png](images/ftplib_dmeo.png)                   
...
f.quit()
```
保存为ftplib\_download.py，运行，看一下结果。
...
[ Windows系统下Python压缩和解压文件、文件夹操作 ](http://blog.csdn.net/ekeuy/article/details/18303555)

[Python札记 -- 文件压缩](http://www.cnblogs.com/PandaBamboo/p/3254449.html)

[python用zipfile模块打包文件或是目录、解压zip文件实例](http://wangwei007.blog.51cto.com/68019/1045577)

[ python解压压缩包的几种方法 ](http://blog.csdn.net/luoshengkim/article/details/46647423)

[ftplib — FTP protocol client](https://docs.python.org/2/library/ftplib.html)
