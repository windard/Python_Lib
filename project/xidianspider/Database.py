...
class Database(object):
    ...
    def __init__(self, host="localhost",user="root",password="",db="",port=3306,charset="utf8", debug=False):
        self.host = host
        self.user = user
        ...
        try:
            ...
            if option.get("where",""):
                ...
                for key,value in option.get("where").items():
                    where.append("%s='%s'"%(unicode(key, 'utf-8'),unicode(value, 'utf-8')))
                conds += "WHERE "
            ...
            if option.get("limit",""):
                ...
                if type(option.get("limit")) == unicode:
                    limit = " LIMIT "+unicode(option.get("limit"), 'utf-8')
                else:
    ...
    def set(self,table,values,option={}):
        try:
            ...
            for key,value in values.items():
                conds.append("%s='%s'"%(unicode(key, 'utf-8'),unicode(value, 'utf-8')))
            if len(where):
